-- Consentimiento del titular al registro público de afiliados (Ley 21.719).
ALTER TABLE public.clientes
  ADD COLUMN IF NOT EXISTS consentimiento_datos_personales jsonb;

COMMENT ON COLUMN public.clientes.consentimiento_datos_personales IS
  'Evidencia de consentimiento del titular al enviar el formulario público de registro (canal, fecha, versión del texto).';
...
BEGIN
  ...
  IF jsonb_typeof(p_row -> 'consentimiento_datos_personales') = 'object' THEN
    v_consentimiento := p_row -> 'consentimiento_datos_personales';
  ELSE
    v_consentimiento := null;
  END IF;

  IF auth.uid() IS NULL THEN
    IF v_consentimiento IS NULL OR coalesce(trim(v_consentimiento ->> 'otorgado_en'), '') = '' THEN
  ...
  IF jsonb_typeof(p_row -> 'causante') = 'object' THEN
    v_causante := p_row -> 'causante';
  ELSE
    v_causante := null;
  END IF;
  ...
  ON CONFLICT (user_id, rut) DO UPDATE SET
    ...
    causante = excluded.causante,
    consentimiento_datos_personales = COALESCE(excluded.consentimiento_datos_personales, clientes.consentimiento_datos_personales),
...
COMMENT ON FUNCTION public.registrar_cliente_por_invite(uuid, jsonb) IS
  'Registro con enlace (?ref): valida p_invite. Sin sesión: exige consentimiento_datos_personales. Con sesión (auth.uid): user_id = asesor logueado.';
